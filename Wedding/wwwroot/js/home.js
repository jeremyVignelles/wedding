...
class HomeGame extends CanvasGame {
    constructor() {
        super();
        this.lastUpdateDate = new Date(2020, 7, 6, 13, 15).getTime();
        this.lastUpdate = `Dernières nouveautés :
        06/08/2020 :
        ...
        this.enterHouseOverlayOpacity = 0;
        ...
        if (!lastVisit) {
            ...
            if (!alreadyVisited) { // Compatibility with the "visit" local storage
    ...
    handleControls() {
        ...
        if (wasNewsDialogOpen && !this.currentDialog) { // The news dialog was displayed, but not anymore, just update the last visit
            this.lastVisit = new Date().getTime();
            window.localStorage.setItem("lastVisit", this.lastVisit.toString());
        }
    }
    ...
    computeAnimations() {
        ...
        if ((this.playerPosition.pos.y < 0 || this.playerPosition.pos.x < -4) && this.enterHouseOverlayOpacity < 1) {
            ...
            if (this.enterHouseOverlayOpacity >= 0.5 && !this.redirected) {
                if (this.playerPosition.pos.y < 0) { // house
    ...
    createForestPattern() {
        ...
        for (var t of trees) {
            canvas.getContext("2d").drawImage(this.sprites, this.spritesDescription.tree.x * squareSize, this.spritesDescription.tree.y * squareSize, this.spritesDescription.tree.width * squareSize, this.spritesDescription.tree.height * squareSize, t.x * squareSize, t.y * squareSize, this.spritesDescription.tree.width * squareSize, this.spritesDescription.tree.height * squareSize);
        }
