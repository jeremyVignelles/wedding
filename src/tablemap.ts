...
export class TableMap {
    ...
    firstNameMapping: {[firstName: string] : string[]} = {};

    seatedGuests: {[tableId: number]: (Guest|null)[]} = {};
    seatedCount = 0;
    
    chairSize = 42;
    svgNamespace = "http://www.w3.org/2000/svg";
    
    constructor(tables: Table[], guests: Guest[]) {
        this.tables = tables;
        this.guests = guests;
        ...
        this.svgManager = new SvgManager("/img/tableMap.svg", document.getElementById("map") as HTMLObjectElement, 0.25, 10, this.onAreaClicked.bind(this));
    ...
    removeGuestFromSeat(guest: Guest) {
        if(guest.tableId !== null && guest.seatNumber !== null && this.seatedGuests[guest.tableId][guest.seatNumber] === guest)
        {// withdraw the guest from its chair
    ...
    refreshTableSeats(g: Guest) {
        var seatSelect = document.getElementById("seat-select-" + g.id) as HTMLSelectElement;
        ...
        while(t.capacity + 1 < seatSelect.options.length)
        {// The new capacity is lower than the current capacity, remove seats
     ...
     */
    refreshSeatDisplay(tableId: number, seatNumber: number) {
        const guest = this.seatedGuests[tableId][seatNumber];
        const svgChair = this.svgManager.svgDoc.getElementById(`chair-${tableId}-${seatNumber}`) as any as SVGElement;
        const svgGuestName = this.svgManager.svgDoc.getElementById(`chair-guest-name-text-${tableId}-${seatNumber}`) as any as SVGElement;
