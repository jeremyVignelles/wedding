...
export class SvgManager {
    ...
    loaded: Promise<never>;

    constructor(file:string, container: HTMLObjectElement, minZoom: number, maxZoom: number, onClick?:(id:string) => void)
    {
        this.minZoom = minZoom;
        this.maxZoom = maxZoom;
        this.onClick = onClick;
        this.container = container;
        this.loaded = new Promise<never>((resolve, reject) => {
    ...
    onAreaClicked(ev: Event)
    {
        if(this.onClick)
        {
            this.onClick((ev.currentTarget as SVGElement).id);
        }
    }
    ...
    attachPanZoomEvents()
    {
        ...
        let pointersCache: Array<{id: number, x:number, y: number}> = [];
        const svgRootElement = this.svgDoc.firstElementChild as SVGSVGElement; 
        const drawingBounds = {
        ...
        let currentPanZoom = {
            ...
            zoom: 
                (svgDimensions.width/svgDimensions.height > drawingBounds.width/drawingBounds.height)
        ...
        svgRootElement.addEventListener('pointerdown', (ev : PointerEvent) => {
        ...
        svgRootElement.addEventListener('pointermove', (ev: PointerEvent) => {
            ev.preventDefault();
        ...
        function removePointer(ev: PointerEvent) {
